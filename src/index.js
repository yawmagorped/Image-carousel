...
const handleCarousel = (() => {
  ...
  const rotateCarousel = (dirString, clankerMode) => {
    if (clankerMode != true) {
      resetTimer();
    }
    ...
    emptyDots();
    if (dirString == "left") {
      if (currIndex == 0) currIndex = imageSources.length - 1;
      else currIndex--;

      setCarouselFocus(currIndex);
    } else if (dirString == "right") {
      if (currIndex == imageSources.length - 1) currIndex = 0;
      else currIndex++;
  ...
  leftArrow.addEventListener("click", () => rotateCarousel("left", false));
  rightArrow.addEventListener("click", () => rotateCarousel("right", false));

  const resetTimer = () => {
    if (timer) {
      clearInterval(timer);
      timer = setInterval(rotateCarousel, 5000, "right", true);
    }
  };
